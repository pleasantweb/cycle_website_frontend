...
const cycledit=async(req: NextApiRequest, res: NextApiResponse<Data>)=>{
    if(req.method === 'PUT'){
        const {id,sellor,bike_name,description,brand,category,bike_image,stock,price,discount,delivery_time,total_sold} = req.body
            ...
            try{
                ...
                if(apiRes.status === 200){
                    return res.status(200).json({
                        error:'Your cycle edited successfully.'
                    })
                }else{
